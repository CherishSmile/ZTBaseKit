...
//  ZTAttributedLabel
//
//  It is adapted by TTTAttributedLabel.
//  For source code to refer to https://github.com/TTTAttributedLabel/TTTAttributedLabel
//
...
//! Project version number for ZTAttributedLabel.
FOUNDATION_EXPORT double ZTAttributedLabelVersionNumber;

//! Project version string for ZTAttributedLabel.
FOUNDATION_EXPORT const unsigned char ZTAttributedLabelVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <ZTAttributedLabel/PublicHeader.h>

@class ZTAttributedLabelLink;

/**
 Vertical alignment for text in a label whose bounds are larger than its text bounds
 */
typedef NS_ENUM(NSInteger, ZTAttributedLabelVerticalAlignment) {
    ZTAttributedLabelVerticalAlignmentCenter   = 0,
    ZTAttributedLabelVerticalAlignmentTop      = 1,
    ZTAttributedLabelVerticalAlignmentBottom   = 2,
};

/**
 Determines whether the text to which this attribute applies has a strikeout drawn through itself.
 */
extern NSString * const kZTStrikeOutAttributeName;

/**
 The background fill color. Value must be a `CGColorRef`. Default value is `nil` (no fill).
 */
extern NSString * const kZTBackgroundFillColorAttributeName;

/**
 The padding for the background fill. Value must be a `UIEdgeInsets`. Default value is `UIEdgeInsetsZero` (no padding).
 */
extern NSString * const kZTBackgroundFillPaddingAttributeName;

/**
 The background stroke color. Value must be a `CGColorRef`. Default value is `nil` (no stroke).
 */
extern NSString * const kZTBackgroundStrokeColorAttributeName;

/**
 The background stroke line width. Value must be an `NSNumber`. Default value is `1.0f`.
 */
extern NSString * const kZTBackgroundLineWidthAttributeName;

/**
 The background corner radius. Value must be an `NSNumber`. Default value is `5.0f`.
 */
extern NSString * const kZTBackgroundCornerRadiusAttributeName;

@protocol ZTAttributedLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol ZTAttributedLabel <NSObject>
...
/**
 `ZTAttributedLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 ## Differences Between `ZTAttributedLabel` and `UILabel`
 
 For the most part, `ZTAttributedLabel` behaves just like `UILabel`. The following are notable exceptions, in which `ZTAttributedLabel` may act differently:
 ...
 `ZTAttributedLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
 ...
 */
@interface ZTAttributedLabel : UILabel <ZTAttributedLabel, UIGestureRecognizerDelegate>
...
/**
 ...
 @discussion A `ZTAttributedLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <ZTAttributedLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `ZTAttributedLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) ZTAttributedLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c ZTAttributedLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c ZTAttributedLabelLink object.
 */
- (void)addLink:(ZTAttributedLabelLink *)link;
 ...
 */
- (ZTAttributedLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
...
/**
 ...
 @return The newly added link object.
 */
- (ZTAttributedLabelLink *)addLinkToURL:(NSURL *)url
...
/**
 ...
 @return The newly added link object.
 */
- (ZTAttributedLabelLink *)addLinkToAddress:(NSDictionary *)addressComponents
                                   withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (ZTAttributedLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (ZTAttributedLabelLink *)addLinkToDate:(NSDate *)date
                                withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (ZTAttributedLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range;
...
/**
 Returns the @c ZTAttributedLabelLink at the give point if it exists.
 
 @discussion This can be used together with @c UIViewControllerPreviewingDelegate to peek into links.
 
 @param point The point inside the label.
 */
- (ZTAttributedLabelLink *)linkAtPoint:(CGPoint)point;

@end

/**
 The `ZTAttributedLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol ZTAttributedLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(ZTAttributedLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `ZTAttributedLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface ZTAttributedLabelLink : NSObject <NSCoding>

typedef void (^ZTAttributedLabelLinkBlock) (ZTAttributedLabel *, ZTAttributedLabelLink *);
...
/**
 ...
 @c ZTAttributedLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) ZTAttributedLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c ZTAttributedLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) ZTAttributedLabelLinkBlock linkLongPressBlock;
...
/**
 ...
 @return The initialized link object.
 */
- (instancetype)initWithAttributesFromLabel:(ZTAttributedLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result;
