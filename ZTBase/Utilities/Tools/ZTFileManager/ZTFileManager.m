...
//  ZTFileManager
//
//  Created by MZY on 15/9/30.
//  Copyright © 2015年 MZY. All rights reserved.
//

#import "ZTFileManager.h"

@interface ZTFileManager()

@property (strong, nonatomic) NSFileManager *manager;

@end

@implementation ZTFileManager
...
//        NSArray *deepArr = [manager subpathsOfDirectoryAtPath:path error:&error];
        NSArray *deepArr = [manager subpathsAtPath:path];
        if (!error) {
            listArr = deepArr;
        }else {
            listArr = nil;
...
+ (BOOL)clearCachesDirectory {
    NSArray *subFiles = [self listFilesInCachesDirectoryByDeep:YES];
    BOOL isSuccess = YES;
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[self cachesDir] stringByAppendingPathComponent:file];
        if ([[NSFileManager defaultManager] fileExistsAtPath:absolutePath]) {
            if ([self isWritableItemAtPath:absolutePath]) {
                isSuccess &= [self removeItemAtPath:absolutePath];
            }
        }
    }
...
+ (NSNumber *)sizeOfDirectoryAtPath:(NSString *)path error:(NSError *__autoreleasing *)error {
    if ([self isDirectoryAtPath:path error:error]) {
        ...
        while (file = [contentsEnumurator nextObject]) {
            ...
            if ([self isWritableItemAtPath:[path stringByAppendingPathComponent:file]]) {
