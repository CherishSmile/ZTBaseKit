...
@interface ZTAddressPicker ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@end
@implementation ZTAddressPicker
...
static CGFloat const TITLEHEIGHT = 49.0;
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        _cancelBtn.titleLabel.font = GetFont(F5);
    }
    return _cancelBtn;
}
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn addTarget:self
           ...
           forControlEvents:UIControlEventTouchUpInside];
        _sureBtn.titleLabel.font = GetFont(F5);
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]initWithFrame:
                              CGRectMake(0, TITLEHEIGHT, SELFSIZE.width, 216)];
...
- (void)setIsAutoOpenLast:(BOOL)isAutoOpenLast {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self->_isAutoOpenLast = isAutoOpenLast;
        if (isAutoOpenLast) {
            ...
            [self getIdx:^(NSInteger p, NSInteger c, NSInteger a) {
                ...
                [weakSelf performSelector:@selector(selectDefaultCity) withObject:nil afterDelay:0.01];
            }];
        } else {
...
-(void)selectDefaultCity{
    [self.addressPickerView selectRow:self.cityIndex inComponent:1 animated:NO];
    [self.addressPickerView reloadComponent:1];
    [self performSelector:@selector(selectDefaultArea) withObject:nil afterDelay:0.01];

}
-(void)selectDefaultArea{
    [self.addressPickerView selectRow:self.areaIndex inComponent:2 animated:NO];
...
- (void)loadAddressData{
    ...
    NSString * filePath = [ZTBaseBundle pathForResource:@"address" ofType:@"txt"];
...
- (void)show{
    ...
    alertView.isTouchOtherUndissmiss = false;
...
- (void)sureBtnClicked{
    ...
    if (self.returnBlock) {
        self.returnBlock(p.name, c.cityName, c.areas[selectArea]);
    }
    [self hide];
}
...
- (void)getIdx:(lastIdxBlock)block {
    NSNumber *p = [[NSUserDefaults standardUserDefaults] valueForKey:AddressProvinceIdxName]?:@(16);
    NSNumber *c = [[NSUserDefaults standardUserDefaults] valueForKey:AddressCityIdxName]?:@(1);
    NSNumber *a = [[NSUserDefaults standardUserDefaults] valueForKey:AddressAreaIdxName]?:@(1);
    
    self.provinceIndex = p.integerValue-1;
    self.cityIndex = c.integerValue-1;
    self.areaIndex = a.integerValue-1;
