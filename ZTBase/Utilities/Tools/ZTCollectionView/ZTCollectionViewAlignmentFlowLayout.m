...
#import "ZTCollectionViewAlignmentFlowLayout.h"

@interface ZTCollectionViewAlignmentFlowLayout()
{
...
@implementation ZTCollectionViewAlignmentFlowLayout

-(instancetype)init{
    return [self initWithAlignment:ZTCollectionViewCellAlignmentCenter betweenOfCellPadding:15.0];
}

-(instancetype)initWithAlignment:(ZTCollectionViewCellAlignment)cellAlignment{
    return [self initWithAlignment:cellAlignment betweenOfCellPadding:15.0];
}
...
-(instancetype)initWithAlignment:(ZTCollectionViewCellAlignment)cellAlignment betweenOfCellPadding:(CGFloat)cellPadding{
    if (self = [super init]) {
        ...
        _betweenOfCellPadding = cellPadding;
        _cellAlignment = cellAlignment;
    }
    return self;
}

-(void)setBetweenOfCellPadding:(CGFloat)betweenOfCellPadding{
    _betweenOfCellPadding = betweenOfCellPadding;
    self.minimumInteritemSpacing = betweenOfCellPadding;
}
...
-(void)setCellFrameWith:(NSMutableArray*)layoutAttributes{
    ...
    switch (_cellAlignment) {
        case ZTCollectionViewCellAlignmentLeft:
            nowWidth = self.sectionInset.left;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
                nowWidth += nowFrame.size.width + self.betweenOfCellPadding;
            }
            _sumCellWidth = 0.0;
            [layoutAttributes removeAllObjects];
            break;
            
        case ZTCollectionViewCellAlignmentCenter:
            nowWidth = (self.collectionView.frame.size.width - _sumCellWidth - ((layoutAttributes.count - 1) * _betweenOfCellPadding)) / 2;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
        ...
        case ZTCollectionViewCellAlignmentRight:
            ...
            for (NSInteger index = layoutAttributes.count - 1 ; index >= 0 ; index-- ) {
                ...
                attributes.frame = nowFrame;
                nowWidth = nowWidth - nowFrame.size.width - _betweenOfCellPadding;
            }
            _sumCellWidth = 0.0;
            [layoutAttributes removeAllObjects];
            break;
