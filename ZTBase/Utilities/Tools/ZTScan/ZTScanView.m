...
//线条扫码动画封装
@property (nonatomic,strong,nullable)ZTScanLineAnimation *scanLineAnimation;
//网格扫码动画封装
@property (nonatomic,strong,nullable)ZTScanNetAnimation *scanNetAnimation;
...
- (void)startScanAnimation
{
    ...
    {
        case ZTScanViewAnimationStyle_LineMove:
        {
            //线动画
            if (!_scanLineAnimation)
            {
                self.scanLineAnimation = [[ZTScanLineAnimation alloc]init];
                if (self.viewStyle.scanLineColor) {
                    self.scanLineAnimation.tintColor = self.viewStyle.scanLineColor;
                }
            }
            ...
            break;
        case ZTScanViewAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation){
                self.scanNetAnimation = [[ZTScanNetAnimation alloc]init];
                if (self.viewStyle.scanLineColor) {
                    self.scanLineAnimation.tintColor = self.viewStyle.scanLineColor;
                }
            }
            ...
            break;
        case ZTScanViewAnimationStyle_LineStill:
        {
            if (!_scanLineStill) {
...
- (void)drawScanRect
{
    ...
    //扫码区域Y轴最小坐标
        ...
        CGFloat YMinRetangle = MAX(self.frame.size.height / 2.0 - sizeRetangle.height/2.0 - _viewStyle.centerUpOffset, UIApplication.sharedApplication.statusBarFrame.size.height+40+20);
  ...
  //画矩形框4格外围相框角
    ...
    {
        case ZTScanViewPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
            break;
        case ZTScanViewPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
            break;
        case ZTScanViewPhotoframeAngleStyle_Inner:
        {           
...
//    CGFloat YMinRetangle = view.frame.size.height / 2.0 - sizeRetangle.height/2.0 - style.centerUpOffset;
        CGFloat YMinRetangle = MAX(view.frame.size.height / 2.0 - sizeRetangle.height/2.0 - style.centerUpOffset, UIApplication.sharedApplication.statusBarFrame.size.height+40+20);
    //扫码区域坐标
