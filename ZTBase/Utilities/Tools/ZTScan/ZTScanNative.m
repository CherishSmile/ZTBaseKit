...
@interface ZTScanNative()<AVCaptureMetadataOutputObjectsDelegate,AVCaptureVideoDataOutputSampleBufferDelegate>
{
...
//扫码结果
@property (nonatomic, strong) NSMutableArray<ZTScanResult*> *arrayResult;
 ...
 *  扫码结果返回
 */
@property(nonatomic,copy)void (^blockScanResult)(NSArray<ZTScanResult*> *array);


@end

@implementation ZTScanNative
...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<ZTScanResult*> *array))block
{
    if (self = [super init]) {
...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType success:(void(^)(NSArray<ZTScanResult*> *array))block
{
    if (self = [super init]) {
...
- (void)initParaWithPreView:(UIView*)videoPreView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<ZTScanResult*> *array))block
{
    ...
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:stillImageConnection
                                                         completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
     {
         ...
         if (imageDataSampleBuffer)
         {
             ...
             for (ZTScanResult* result in self.arrayResult) {
...
- (void)takePhoto:(void (^)(UIImage *))completion{
    AVCaptureConnection *stillImageConnection = [self connectionWithMediaType:AVMediaTypeVideo fromConnections:[[self stillImageOutput] connections]];
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:stillImageConnection
                                                         completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
     {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.scanCameraBrightnessHandler) {
            self.scanCameraBrightnessHandler(brightnessValue, brightnessThresholdValue);
        }
    });
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    for(AVMetadataObject *current in metadataObjects)
    {
        if ([current isKindOfClass:[AVMetadataMachineReadableCodeObject class]] )
        {
            ...
            {
                ZTScanResult *result = [ZTScanResult new];
...
#pragma mark --识别条码图片
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<ZTScanResult*> *array))block;
{
    if ([[[UIDevice currentDevice]systemVersion]floatValue] < 8.0 )
    {
        if (block) {
            ZTScanResult *result = [[ZTScanResult alloc]init];
    ...
    NSMutableArray<ZTScanResult*> *mutableArray = [[NSMutableArray alloc]initWithCapacity:1];
    for (int index = 0; index < [features count]; index ++)
    {
        ...
        ZTScanResult *item = [[ZTScanResult alloc]init];
