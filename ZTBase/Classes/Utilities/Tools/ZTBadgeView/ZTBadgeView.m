...
// Soon ZTBadgeView will require iOS 7 and we'll be able to use the new methods.
    ...
    #define ZTBadgeViewSilenceDeprecatedMethodEnd()     _Pragma("clang diagnostic pop")
#else
    #define ZTBadgeViewSilenceDeprecatedMethodStart()
    #define ZTBadgeViewSilenceDeprecatedMethodEnd()
#endif

static const CGFloat ZTBadgeViewShadowRadius = 1.0f;
static const CGFloat ZTBadgeViewHeight = 16.0f;
static const CGFloat ZTBadgeViewTextSideMargin = 8.0f;
static const CGFloat ZTBadgeViewCornerRadius = 10.0f;

// Thanks to Peter Steinberger: https://gist.github.com/steipete/6526860
static BOOL ZTBadgeViewIsUIKitFlatMode(void)
{
...
{
    ZTBadgeView *badgeViewAppearanceProxy = ZTBadgeView.appearance;

    badgeViewAppearanceProxy.backgroundColor = UIColor.clearColor;
    badgeViewAppearanceProxy.badgeAlignment = ZTBadgeViewAlignmentTopRight;
...
+ (void)initialize
{
    if (self == ZTBadgeView.class)
    {
        [self applyCommonStyle];

        if (ZTBadgeViewIsUIKitFlatMode())
        {
            [self applyIOS7Style];
        }
        else
...
- (void)layoutSubviews
{
    ...
    const CGFloat viewWidth = MAX(_badgeMinWidth, textWidth + ZTBadgeViewTextSideMargin + (marginToDrawInside * 2));
    const CGFloat viewHeight = ZTBadgeViewHeight + (marginToDrawInside * 2);
    ...
    switch (self.badgeAlignment) {
        case ZTBadgeViewAlignmentTopLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZTBadgeViewAlignmentTopRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZTBadgeViewAlignmentTopCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZTBadgeViewAlignmentCenterLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case ZTBadgeViewAlignmentCenterRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case ZTBadgeViewAlignmentBottomLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZTBadgeViewAlignmentBottomRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZTBadgeViewAlignmentBottomCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZTBadgeViewAlignmentCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        default:
            NSAssert(NO, @"Unimplemented ZTBadgeAligment type %lul", (unsigned long)self.badgeAlignment);
    }
...
{
    ZTBadgeViewSilenceDeprecatedMethodStart();
    return [self.badgeText sizeWithFont:self.badgeTextFont];
    ZTBadgeViewSilenceDeprecatedMethodEnd();
}

#pragma mark - Setters

- (void)setBadgeAlignment:(ZTBadgeViewAlignment)badgeAlignment
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw)
    {
        ...
        UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(ZTBadgeViewCornerRadius, ZTBadgeViewCornerRadius)];
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextSetShadowWithColor(ctx, self.badgeShadowSize, ZTBadgeViewShadowRadius, self.badgeShadowColor.CGColor);
