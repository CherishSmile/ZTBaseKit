...
NSString *completeRPCURLPath = @"/ztwebviewprogressproxy/complete";

const float ZTInitialProgressValue = 0.1f;
const float ZTInteractiveProgressValue = 0.5f;
const float ZTFinalProgressValue = 0.9f;

@implementation ZTUIWebViewProgress
{
...
- (void)startProgress
{
    if (_progress < ZTInitialProgressValue) {
        [self setProgress:ZTInitialProgressValue];
    }
}
...
- (void)incrementProgress
{
    ...
    float maxProgress = _interactive ? ZTFinalProgressValue : ZTInteractiveProgressValue;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    BOOL isNotUrl = !([_currentURL.absoluteString.lowercaseString hasPrefix:@"http://"]||[_currentURL.absoluteString.lowercaseString hasPrefix:@"https://"]);
    if ((complete && isNotRedirect)||isNotUrl) {
        [self completeProgress];
    }
}
