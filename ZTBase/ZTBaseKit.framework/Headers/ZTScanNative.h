...
/**
 ios系统自带扫码功能
 */
@interface ZTScanNative : NSObject
...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect
              success:(void(^)(NSArray<ZTScanResult*> *array))block;
 ...
 *  光线检测回调
 */
@property(nonatomic,copy)void (^scanCameraBrightnessHandler)(CGFloat brightnessValue,CGFloat brightnessThresholdValue);
...
/**
 拍照
 */
- (void)takePhoto:(void(^)(UIImage * image))completion;
